...
class GCash(PaymentMethod):
    def __init__(self, account_name, balance, phone_number, email, otp_verification):
        super().__init__(account_name, balance)
        self._phone_number = phone_number
        self.email = email
        self.__otp_verification = otp_verification

    def pay(self, amount):
        if self.is_balance_sufficient(amount):
            self.log_transaction("PAYMENT", "GCash")
            self._balance -= amount
            print(f"Input OTP to verify payment: {self.__otp_verification}")
...
# GCash 
payment1 = GCash("J***el P.", 200, "09953167434", "kimjimuelperez@gmail.com", "654389")
payment1.pay(100)
payment1.refund(50)
