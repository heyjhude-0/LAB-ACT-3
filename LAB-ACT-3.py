...
class BankTransfer(PaymentMethod):
    ...
    def pay(self, amount):
        ...
        if self.is_balance_sufficient(amount):
            self._balance -= amount
            print(f"Paid ₱{amount} via Bank Transfer from {self.bank_name}. New balance: ₱{self._balance}")
        else:
            print("Payment error: Insufficient balance.")
    ...
    def refund(self, amount):
        ...
        self._balance += amount
        print(f"Refunded ₱{amount} to Account Number #{self.account_number}. New balance: ₱{self._balance}")
